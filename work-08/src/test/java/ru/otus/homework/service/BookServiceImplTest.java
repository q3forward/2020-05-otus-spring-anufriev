...
class BookServiceImplTest {
    ...
    void findByIdTest() {
        Optional<Book> book = Optional.of(new Book("1", "Test Book", author, genre));
        given(repo.findById(anyString())).willReturn(book);

        Book actual = service.getById("1");
    ...
    void findAllTest() {
        ...
        Book book2 = new Book("2", "Test Book", author, genre);
    ...
    void updateTest() throws BookNotFoundException {
        ...
        List<Author> authorList = Collections.singletonList(author);
        given(repo.findById(anyString())).willReturn(oldBook);
        given(authorService.findByName(any())).willReturn(authorList);
        given(genreService.findByName(any())).willReturn(genre);
